...
def merge_protein_emb_files(output_dir, out_path, fasta_file, prot_embed_no) :
    ...
    for k, fasta in enumerate(fasta_sequences) :
        if k % prot_embed_no == 0 and k > 0 :
            torch.save(new_dict,join(out_path, "Protein", "ESM2_650M", "Protein_embeddings_V"+str(version+1)+".pt"))
            new_dict = {}
            version +=1
...
def create_protein_embeddings_ESM2_650M(unique_sequences, out_path, prot_embed_no) :
    if not os.path.exists(join(out_path, "Protein", "ESM2_650M")) :
        os.makedirs(join(out_path, "Protein", "ESM2_650M"), exist_ok=True)
    ...
    output_dir = join(out_path, "Protein", "ESM2_650M", "temp")
